        ...
        {loading ? (
          [1, 2, 3].map((i) => (
            <div key={i} className="pm-skeleton-group">
              <div
                className="pm-skeleton-bar"
                style={{ width: "180px", height: "14px" }}
              />
              <div
                className="pm-skeleton-bar"
                style={{ width: "140px", height: "12px", marginLeft: "28px" }}
              />
              <div
                className="pm-skeleton-bar"
                style={{ width: "160px", height: "12px", marginLeft: "28px" }}
              />
            </div>
          ))
        ) : grouped.length === 0 ? (
          search ? (
            ...
            <div className="pm-empty-state">
              ...
              <p className="pm-empty-desc">
                Create positions and assign them to departments.
              </p>
              <button
                className="pm-btn pm-btn-primary"
                onClick={() => openCreatePanel("")}
          ...
          grouped.map((dept) => {
            ...
            return (
              <div
                key={dept.id}
                className={`pm-dept-group${isOpen ? " pm-dept-group--open" : ""}`}
              >
                {/* Department başlığı */}
                <div
                  className="pm-dept-header"
                  onClick={() => toggleCollapse(dept.id)}
                >
                  <span
                    className={`pm-chevron${isOpen ? " pm-chevron--open" : ""}`}
                  >
                    ▶
                  </span>
                  <div
                    className="pm-dept-icon"
                    style={{ background: getAvatarColor(dept.name) }}
                  >
                  ...
                  <button
                    className="pm-dept-add-btn"
                    title="Add position"
                    onClick={(e) => {
                      e.stopPropagation();
                      openCreatePanel(dept.id);
                    }}
                  >
                    <svg
                      width="12"
                    ...
                    {posCount === 0 ? (
                      ...
                      dept.positions.map((pos, idx) => {
                        ...
                        return (
                          <div
                            key={pos.id}
                            className={`pm-pos-row${isDeleteConfirm ? " pm-pos-row--confirm" : ""}${isLast ? " pm-pos-row--last" : ""}`}
                          >
                            {/* Tree konnektoru */}
                            <div className="pm-tree-line">
                              <div
                                className={`pm-tree-vert${isLast ? " pm-tree-vert--last" : ""}`}
                              />
                            ...
                            {isDeleteConfirm ? (
                              <div className="pm-row-delete-confirm">
                                ...
                                <button
                                  className="pm-delete-yes"
                                  onClick={() => handleDelete(pos)}
                                >
                                  Yes
                                </button>
                                <button
                                  className="pm-delete-no"
          ...
          <div className="pm-form-panel">
            ...
            <form className="pm-form-body" onSubmit={handleSubmit}>
              <div className="pm-form-field">
                <label className="pm-form-label pm-form-label--required">
                  Position Name *
                </label>
                <input
                ...
                />
                {formError && (
                  <span className="pm-form-error">{formError}</span>
                )}
              </div>
              <div className="pm-form-field">
                <label className="pm-form-label pm-form-label--required">
